...
# !!!Variables to set==========================================================
# =============================================================================
folder_path = 'C:/Users/mbrouwer/OneDrive - NIOZ/Documenten/GitHub/molecular_tools/qPCR_CFX/test' # Directory where the CSV files are located
std_curve_copies = 4.06 # What is the copies/µL *10^x of the standard dilution series
PCR_that_has_std_curve = 1 # Which PCR contains your standard dilution series
...
def remove_outliers(dataframe, column):
    ...
    # Calculate IQR (nterquartile range)
    ...
    # Make list with STDs to use, excluding outliers
    non_outlier_values = []
    for STD in dataframe[column]:
        if low_outlier_limit <= STD <= high_outlier_limit:
            non_outlier_values.append(STD)
    return non_outlier_values

# Get normalization values from PCR with STD curve
# Get DataFrame from the PCR with the STD curves
df_normalization_PCR = PCRs.get('PCR_' + str(PCR_that_has_std_curve))
# Get STD cqs from the PCR with the STD curve
normalization_STDs = df_normalization_PCR[df_normalization_PCR['Sample'] == 'STD']
# Remove NaNs and outliers
normalization_STDs = remove_outliers(normalization_STDs, 'Cq')
# Calculate mean STD of normalization PCR
mean_normalization_STD =  statistics.mean(normalization_STDs)

# Calculate normalization factors for all PCRs
# Create an empty dataframe for the standards of all PCRs
...
for PCR_name, PCR_df in PCRs.items():
    ...
    # Remove outliers
    STDs = remove_outliers(STDs_from_df, 'Cq')
    # Add STDs to STDs dataframe 
...
STDs_df["Correction"] = STDs_df["Mean"] - mean_normalization_STD


for PCR_name, PCR_df in PCRs.items():-
    # normalization_factor = statistics.mean(STDs) - mean_normalization_STD

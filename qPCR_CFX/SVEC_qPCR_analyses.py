...
# !!!Variables to set==========================================================
# =============================================================================
folder_path = 'C:/Users/rdebeer/OneDrive - NIOZ/Data/python tests/Test_qPCR' # Directory where the CSV files are located
std_curve_copies = 4.06 # What is the copies/µL of the standard *10^x
analytical_run_code = 'NIOZ100' # Analytical run code generated by DAS
# =============================================================================
# Import statements ===========================================================
# =============================================================================
...
for PCR_name, PCR_df in PCRs.items():
    if PCR_name == 'PCR_1':
        ...
        STD_correction = average_STD_PCR1 - statistics.mean(STDs_from_df['Cq'])
...
dilutions = pd.read_csv((folder_path +'/'+ analytical_run_code +'_dilution_rates.csv'), usecols=['Sample', 'Dilution'])

# Merges the 2 dataframes and overwrites the sample_calculations column
sample_calculations = pd.merge( dilutions, sample_calculations, on = 'Sample', how='outer')
...
for i, column in enumerate(sample_calculations.columns[sample_calculations.columns.str.startswith('Corrected')], start=1):
    ...
    ax2.scatter(x=sample_names_list, y=result, color=color, label=f"Cq {i}", marker='*')
    
# Specify x-axis and y-axis labels 
ax2.set_xticklabels(sample_names_list, rotation=90)  # Set x-axis labels with rotation
ax2.set_ylabel('Cq value')  # Set y-axis label
ax2.set_title('Cq difference between corrected Cq and Mean')  # Set title for the plot
